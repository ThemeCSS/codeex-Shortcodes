<?php
    ...
    $out = "<ul class='$style'>" . do_shortcode($content);
    $out .= "</ul>";
    return $out;
	}
    ...
    extract( shortcode_atts( array( 'style' => 'clearfix' ), $atts ) );

    $out = "<div class='$style'>" . do_shortcode($content);
    $out .= "</div>";
    return $out;
	}
	...
	function typo_single_image($atts) {
		extract(
			shortcode_atts(array(
				'src' => '', 'width' => '', 'height' => '', 'title' => '', 'style' => ''
			), $atts)
		);

		if($width == '' && $height == '') return "<img src='$src' alt='$title' class='$style' />";

		$theme = get_template_directory_uri();
		$out = "<a href='$src' class='large-image-popup'><img src='$src' height='$height' width='$width' title='$title' alt='$title' class='$style' /></a>";
		return $out;
	}
	add_shortcode( 'typo_image', 'typo_single_image' );
	...
	function typo_jtabs( $atts, $content ){
		extract( shortcode_atts( array( 'anchor' => '%d',	'title' => 'Tab %d' ), $atts ) );
	...
	function typo_toggle_shortcode( $atts, $content = null ) {
		...
		$out = '<h4 class="toggle"><a href="#">'. $title .'</a></h4><div class="toggle_container">' . do_shortcode($content) . '</div>';
		return $out;
	}
	...
	function typo_video_shortcode( $atts, $content = null ) {
	 	extract( shortcode_atts( array( 'site' => 'youtube', 'id' => '', 'w' => '100%', 'h' => '400' ), $atts ) ); 

    if( 'youtube' === $site ) : $src = 'https://www.youtube-nocookie.com/embed/'.$id.'?rel=0'; 
    elseif( 'vimeo' === $site ) : $src = 'https://player.vimeo.com/video/'.$id; 
    elseif( 'dailymotion' === $site ) : $src = 'http://www.dailymotion.com/embed/video/'.$id; 
    endif;

    if ( $id != '' ) :
    	return '<iframe width="'.$w.'" height="'.$h.'" src="'.$src.'" class="video iframe-'.$site.'"></iframe>';
    endif; 
	}
	...
	function typo_google_map( $atts, $content = null ) {
   	extract( shortcode_atts( array( 'width' => '653', 'height' => '284', 'src' => '' ), $atts ) );
    
    $out = '<iframe width="'.$width.'" height="'.$height.'" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="'.$src.'&amp;output=embed"></iframe>';
    return $out;
	}
	add_shortcode( 'typo_googlemap', 'typo_google_map' );


	/* Social Icons */
	
	function typo_social_icons( $atts, $content = null ) {
		extract( shortcode_atts( array( 'href' => '', 'style' => '', 'target' => '', 'type' => '' ), $atts ) );
	...
	function typo_callout( $atts, $content = null ) {
		extract( shortcode_atts( array(	'style' => 'typo_default', 'type' => '' ), $atts ) );
    ...
    $out = "<label>" . do_shortcode($content) . "</label>";
    return $out;
	}
	...
	function typo_skills_bar( $atts, $content = null ) {
		extract( shortcode_atts( array( 'percent' => '', 'color' => '' ), $atts ) );
